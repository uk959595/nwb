...
#define  SYSTEM_MEMORY_ADDRESS_X32 0x70000000000
...
std::vector <DWORD> AobScan::FindSigX32(HANDLE hProcess, const char* Value, ULONG32 Start, ULONG32 End)
{
...
std::vector <ULONG64> AobScan::FindSigX64(const char* Value, DWORD procID, ULONG64 Start, ULONG64 End)
{
    ...
    while (VirtualQueryEx(hProcess, (LPVOID)Start, &mem_info, sizeof(mem_info)))
    {
        if (mem_info.Protect != 16 && mem_info.RegionSize != 1 && mem_info.Protect != 512)
        {
            ...
            {
                Start = (ULONG64)mem_info.BaseAddress + (ULONG64)mem_info.RegionSize;
            ...
            if (ReadProcessMemory(hProcess, (LPVOID)Start, buf, dwSectionSize, &nNumberOfBytesRead))
            {
                vector<ULONG64> dwValue = AobScan::SundayHexV(buf, dwSectionSize, (char*)Value);
                for (size_t i = 0; i < dwValue.size(); i++)
                {
